...
export default function QuotationViewPage() {
  ...
  const handleExport = async () => {
    ...
    try {
      ...
      link.download = `${quotation?.number ?? "quotation"}-${quotation?.customer?.username ?? "customer"}.png`;
      link.href = dataUrl;
      link.click();
      toast.success("Quotation exported as image!");
    } catch {
  ...
  const handleShare = async () => {
    ...
    try {
      ...
      const file = new File([blob], `${quotation?.number ?? "quotation"}-${quotation?.customer?.username ?? "customer"}.png`, { type: "image/png" });
      if (navigator.canShare && navigator.canShare({ files: [file] })) {
        await navigator.share({ files: [file], title: `Quotation ${quotation?.number} for ${quotation?.customer?.name}` });
      } else {
        const link = document.createElement("a");
  ...
  if (quotation === null) {
    return <div className="text-center py-12 text-muted-foreground">Quotation not found.</div>;
  }

  const { customer } = quotation;
  const status = STATUS_STYLE[quotation.status] ?? { label: quotation.status, bg: "#f3f4f6", color: "#6b7280" };

  return (
    <div className="space-y-4 max-w-2xl mx-auto">
      <div className="flex items-center gap-3">
        <Button variant="ghost" size="icon" className="cursor-pointer" onClick={() => navigate("/quotations")}>
          <ArrowLeft className="w-4 h-4" />
        </Button>
        <h1 className="text-xl font-bold">Quotation {quotation.number}</h1>
        ...
        >
          ...
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: "24px" }}>
            <div style={{ display: "flex", alignItems: "center", gap: "10px" }}>
              ...
              <div>
                ...
                <p style={{ fontSize: "12px", color: "#6b7280", margin: "2px 0 0" }}>Quotation</p>
              </div>
            </div>
            <div style={{ textAlign: "right" }}>
              <p style={{ fontSize: "11px", color: "#6b7280", margin: 0 }}>Quotation #</p>
              <p style={{ fontSize: "13px", fontWeight: 600, color: "#1a1a2e", margin: "4px 0 0", wordBreak: "break-word" }}>{quotation.number}</p>
            </div>
          </div>
          ...
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-end", marginBottom: "24px" }}>
            <div>
              ...
              <p style={{ fontSize: "16px", fontWeight: 700, color: "#1a1a2e", margin: "4px 0 0" }}>{formatDate(quotation.date)}</p>
              {quotation.validUntil && (
                <>
                  <p style={{ fontSize: "12px", color: "#6b7280", margin: "8px 0 0" }}>Valid Until</p>
                  <p style={{ fontSize: "14px", fontWeight: 600, color: "#1a1a2e", margin: "4px 0 0" }}>{formatDate(quotation.validUntil)}</p>
                </>
              )}
            </div>
            <span style={{
          ...
          <div>
            ...
            <div style={{ background: "#1d2340", borderRadius: "22px", padding: "22px", marginTop: "16px", display: "flex", justifyContent: "space-between", alignItems: "center", color: "#fff" }}>
              ...
              <p style={{ margin: 0, fontSize: "26px", fontWeight: 800 }}>{formatAmount(quotation.total, settings)}</p>
            </div>
          </div>
          ...
          {quotation.note ? (
            <div style={{ marginTop: "20px", background: "#fff8e6", borderRadius: "18px", padding: "16px" }}>
              ...
              <p style={{ margin: 0, fontSize: "14px", color: "#374151", wordBreak: "break-word" }}>{quotation.note}</p>
            </div>
          ) : null}
